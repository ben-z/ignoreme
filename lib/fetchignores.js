 ...
 * @param {fetchIgnoresCallback} callback that handles the contents of the
 *                               returned file.
 */
module.exports = function fetchIgnores(gitignoreLib, fileType, callback) {
  fs.readdir(path.join(__dirname, gitignoreLib), (err, files) => {
    ...
    const done = runAfter(files.length, () => {
      if (dirs.length === 0) {
        winston.debug(`Calling the empty callback for fileType ${fileType} `+
                      `and lib ${gitignoreLib}`);
        callback();
      } else {
        winston.debug(`Fetching subdir ignores for fileType ${fileType} `+
                      `and lib ${gitignoreLib}`);
    ...
    for (const filename of files) {
      ...
      fs.lstat(filePath, (err, stats) => {
        ...
        if (stats.isFile() &&
            filename.toLowerCase() === `${fileType.toLowerCase()}.gitignore`) {
